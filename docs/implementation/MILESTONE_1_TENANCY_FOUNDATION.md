...
| `T-TEN-003` | Later outbox/job, preview-artifact, and download-authorization rows preserve workspace context; browser reads exclude provider coordinates and a service-only loader resolves only one audited authorization; worker logs preserve validated workspace IDs without payloads. | Source integrated for jobs and preview files; live RLS/Storage/log proof remains. |
